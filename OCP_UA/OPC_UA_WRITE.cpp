...
void FORTE_OPC_UA_WRITE::executeEvent(int pa_nEIID){
  switch(pa_nEIID){
    case scm_nEventREQID:
        ...
        TStringId sourceId = sourceFB->getFBTypeId();

        const SFBInterfaceSpec* interfaceSpec = sourceFB->getFBInterfaceSpec();
        TStringId sourceDataPointNameId  = interfaceSpec->m_aunDOName[sourcePoint.mPortId];


//        DEVLOG_ERROR("Zdrojom dat je %s!", source_type);
